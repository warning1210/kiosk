...
CREATE TABLE `event` (
  ...
  `event_type` ENUM ('MONTHLY_FLAVOR', 'HQ_FLAVOR_DISCOUNT', 'FLAVOR_DISCOUNT') NOT NULL COMMENT 'MONTHLY_FLAVOR=본점이 맛을 직접 지정, 할인 없이 사이즈업 필수(전 지점 자동 적용), HQ_FLAVOR_DISCOUNT=본점이 맛과 할인을 둘 다 직접 지정(전 지점 자동 적용), FLAVOR_DISCOUNT=할인값만 본점이 정하고 어느 맛에 붙일지는 지점이 선택(event_branch_flavor)',
  `benefit_type` ENUM ('DISCOUNT_RATE', 'DISCOUNT_AMOUNT') COMMENT 'FLAVOR_DISCOUNT 전용 - MONTHLY_FLAVOR는 할인이 없어 NULL',
  `image_url` varchar(500),
  `description` text,
  `discount_rate` decimal(5,2),
  `discount_amount` int,
  `flavor_id` bigint COMMENT 'MONTHLY_FLAVOR 전용 - 본점이 직접 지정한 할인 맛',
  `size_up_from_product_id` bigint COMMENT 'MONTHLY_FLAVOR 전용(선택) - 이 맛을 고르면 사이즈업할 수 있는 원래 상품',
  `size_up_to_product_id` bigint COMMENT 'MONTHLY_FLAVOR 전용(선택) - 사이즈업 후 상품',
