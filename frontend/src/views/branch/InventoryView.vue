...
async function completeDelivery(item) {
  await http.post(`/branch/inventory/${item.inventoryId}/receive`)
...
async function applyAdjustment() {
  ...
  await http.patch(`/branch/inventory/${item.inventoryId}`, { type: adjustType.value, grams: String(adjustGram.value) })
...
async function loadServerInventory() {
  try {
    const [inventoryResponse, orderResponse] = await Promise.all([http.get('/branch/inventory'), http.get('/branch/orders')])
    inventories.value = inventoryResponse.data.map(item => ({
      id: item.flavorId, inventoryId: item.inventoryId, name: item.flavorName, imageUrl: item.imageUrl || '', remainingG: item.remainingG,
