...
<script setup>import{ref}from'vue';import{useRouter}from'vue-router';import{setPersistence,browserLocalPersistence,browserSessionPersistence,signInWithEmailAndPassword}from'firebase/auth';import{firebaseAuth}from'../../firebase';import http from'../../api/http';const router=useRouter(),loginId=ref(''),password=ref(''),remember=ref(true),error=ref('');async function login(){error.value='';try{const identity=(await http.get(`/branch-auth/login-identity/${encodeURIComponent(loginId.value)}`)).data;await setPersistence(firebaseAuth,remember.value?browserLocalPersistence:browserSessionPersistence);const credential=await signInWithEmailAndPassword(firebaseAuth,identity.email,password.value);const{data}=await http.post('/branch-auth/firebase-session',{idToken:await credential.user.getIdToken(true)});localStorage.setItem('branch-session',JSON.stringify(data));router.replace('/branch/dashboard')}catch(e){console.error(e);error.value=e.response?.data?.message||firebaseMessage(e.code)||'로그인할 수 없습니다.'}}function firebaseMessage(code){return({ 'auth/invalid-credential':'아이디 또는 비밀번호가 올바르지 않습니다.','auth/too-many-requests':'로그인 시도가 너무 많습니다. 잠시 후 다시 시도하세요.','auth/network-request-failed':'네트워크 연결을 확인하세요.'})[code]}</script>
