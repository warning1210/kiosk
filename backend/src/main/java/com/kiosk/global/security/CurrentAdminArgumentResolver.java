...
public class CurrentAdminArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
        ...
        if (header == null || header.isBlank()) {
            throw new BusinessException(ErrorCode.UNAUTHENTICATED, HEADER_NAME + " 헤더가 필요합니다");
        }
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.UNAUTHENTICATED, HEADER_NAME + " 값이 올바르지 않습니다");
        }

        Admin admin = adminRepository.findByIdWithBranch(adminId)
                .orElseThrow(() -> new BusinessException(ErrorCode.UNAUTHENTICATED, "존재하지 않는 관리자입니다"));

        if (admin.getAccountStatus() != AccountStatus.ACTIVE) {
            throw new BusinessException(ErrorCode.UNAUTHENTICATED, "비활성화된 관리자 계정입니다");
        }
