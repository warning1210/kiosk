    ...
    public OrderCheckoutResponse checkout(OrderCheckoutRequest request) {
        ...
        for (ResolvedItem item : resolved) {
            OrderItem orderItem = OrderItem.builder()
                    ...
                    .spoonCount(item.request().spoonCount() != null ? item.request().spoonCount().byteValue() : (byte) 0)
                    .dryIceMinutes(item.request().dryIceMinutes() != null ? item.request().dryIceMinutes().byteValue() : null)
                    .build();
